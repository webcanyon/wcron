...
namespace WebCanyon\WCronBundle\Manager;
 ...
 * Class CronManager
 *
 * @package WebCanyon\WCronBundle\Manager
 */
class CronManager
{
    /**
    ...
    function __construct()
    {
        ...
        foreach ($lines as $lineNumber => $line) {
            ...
            if ((\strpos($line, '#suspended: ', 0) === 0 || 0 !== \strpos($line, '#', 0)) && !preg_match('/^\s*[a-z0-9_]+\s*=/i', $line)) {
                try {
                    $line = CronEntity::createFromString($line);
                } catch (\Exception $e) {
                    $process->addErrorOutput('CronManager was unable to create crontab at line '.$lineNumber);
                }
            }
    ...
    public function get()
    {
        return \array_filter($this->lines, function($line){
            return $line instanceof CronEntity;
        });
    }
    /**
     ...
     */
    public function add(CronEntity $cron)
    {
        $this->lines[] = $cron;
        $this->write();
    }
     ...
     *
     * @param $index string The line number
     */
